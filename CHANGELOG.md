...
- Node frames and zone rectangles now wrap their contents exactly. They were
  sized from a rough per-socket height estimate, which undershot tall nodes
  (two-line headers, vector X/Y/Z blocks), letting node bottoms stick out of
  their frame. The rectangles are now re-derived from React Flow's measured
  node sizes once the nodes have rendered.
